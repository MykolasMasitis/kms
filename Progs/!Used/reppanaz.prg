PROCEDURE RepPanaz
 IF kol_pv!=1
  MESSAGEBOX('ДЛЯ ФОРМИРОВАНИЯ ОТЧЕТА'+CHR(13)+CHR(10)+;
 ...
 IF OpenFile("&pBase\&pvid(1,1)\kms", "kms", "SHARED", "vs")>0
  ...
  RETURN 
 ENDIF 
 IF OpenFile("&pBase\&pvid(1,1)\adr50", "adr50", "SHARED", 'recid')>0
  IF USED('adr50')
   USE IN adr50
  ENDIF 
  IF USED('kms')
  ...
  RETURN 
 ENDIF 
 IF OpenFile("&pBase\&pvid(1,1)\adr77", "adr77", "SHARED", 'recid')>0
  IF USED('adr50')
   USE IN adr50
  ENDIF 
  IF USED('adr77')
