...
* m.fio_paz = ALLTRIM(m.fam) + ' ' + ALLTRIM(m.im) + ' ' + ALLTRIM(m.ot)
 ...
 omark = odoc.GetBookMarks.GetByName('smoname2').GetAnchor
 omark.SetString(ALLTRIM(m.qname))

 omark = odoc.GetBookMarks.GetByName('sn_card').GetAnchor
 omark.SetString(ALLTRIM(m.enp))

 m.IsEnp = IIF(EMPTY(m.enp), 'V', '')
 omark = odoc.GetBookMarks.GetByName('isenp').GetAnchor
 omark.SetString(ALLTRIM(m.IsEnp))
