PROCEDURE efile2kms0
 IF MESSAGEBOX("ХОТИТЕ ПРИНЯТЬ ОБМЕННЫЕ ФАЙЛЫ?", 4+32, 'СТАРЫЙ ВАРИАНТ') != 6
  RETURN 
 ENDIF 
...
FUNCTION ProcOneEFile
 ...
 tnResult = tnResult + OpenFile("&lppath\predst", "predst", "shared", "fio")
 ...
 SCAN
  nProceed = nProceed + 1
  SCATTER MEMO MEMVAR 
  RELEASE permid,ofioid,odocid,osmoid,predstid,wrkid,predstid
...
*   ENDIF 
  ...
  m.fio = LEFT(m.pr_fam,25) + LEFT(m.pr_im,3) + LEFT(m.pr_ot,3)
  m.predstid = 0 
  ...
  ELSE 
   ...
   (m.pr_fam,m.pr_im,m.pr_ot,m.prc_doc,m.prs_doc,m.prn_doc,m.prd_doc,m.prpodr,m.prtel,m.prtel2,m.prpinf)
  ...
  AND m.pricin='П01' && 06/03/14 по просьбe Тани!
   m.pricin='П06'
  ENDIF 
   ...
   DO CASE 
    CASE m.jt=='2'
     IF !EMPTY(m.enp)
      ...
      ELSE 
       ...
       REPLACE kms.dp WITH m.dp, kms.jt WITH m.jt,  kms.scn WITH m.scn, kms.status WITH 1, kms.d_type4 WITH IIF(m.jt!='2', 1, 0)
     ...
     IF !EMPTY(m.vs)
      ...
      ELSE 
       ...
       REPLACE kms.pricin WITH '', kms.tranz WITH '', kms.p_doc WITH 2, ;
        kms.is2fio WITH '0', kms.is2doc WITH '0', kms.scn WITH 'POK', ;
        kms.d_type4 WITH 0, kms.p_doc WITH 3
      ENDIF 
     ELSE 
      INSERT INTO errcur ;
       (vs, enp, fam, im, ot, dr, cmnt);
    ...
    OTHERWISE 
     IF !EMPTY(m.vs)
      ...
      ENDIF 
     ELSE 
*      IF !m.jt != 'k' Изменено про просьбе Останиной 18.11.2014 г.
      IF !INLIST(m.jt,'k','0')
      INSERT INTO errcur ;
       (vs, enp, fam, im, ot, dr, cmnt);
        VALUES;
