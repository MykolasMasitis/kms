...
* Процедура произвольного поиска
 ...
 cmdAll = 'CREATE INDEX recid ON answers (recid)' && Внимание! В исходном файле INT(VAL(recid))!
 IF SQLEXEC(nHandl, cmdAll) = -1
  =AERROR(errarr)
...
* IF SQLEXEC(nHandl, cmdAll) = -1
*  =SQLDISCONNECT(nHandl)
*  MESSAGEBOX("Индекс fam не создан!", 0+64, "kms")
*  RETURN .F.
* ENDIF 
...
* IF SQLEXEC(nHandl, cmdAll) = -1
*  =SQLDISCONNECT(nHandl)
*  MESSAGEBOX("Индекс im не создан!", 0+64, "kms")
*  RETURN .F.
* ENDIF 
...
* IF SQLEXEC(nHandl, cmdAll) = -1
*  =SQLDISCONNECT(nHandl)
*  MESSAGEBOX("Индекс ot не создан!", 0+64, "kms")
*  RETURN .F.
* ENDIF 
