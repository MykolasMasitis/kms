...
PUBLIC oApp, goPSD, nHandl && goPSD - класс дистанционной выгрузки
...
* _SCREEN.WindowState = 2
 ...
 IF fso.FileExists(pBase+'\ShutDown.txt')
  MESSAGEBOX('ЗАПУСК ПРИЛОЖЕНИЯ НЕВОЗМОЖЕН!'+CHR(13)+CHR(10)+;
   'ПРОВОДЯТСЯ ПРОФИЛАКТИЧЕСКИЕ РАБОТЫ.'+CHR(13)+CHR(10)+;
   'ПРИНОСИМ ИЗВИНЕНИЯ ЗА ДОСТАВЛЕННЫЕ НЕУДОБСТВА!',0+64,'')
  =ExitProg()
 ENDIF 
...
* SET LIBRARY TO vfpcompression.fll ADDITIVE 
 ...
 _SCREEN.Caption = 'ЕДИНЫЙ ПУНКТ ВЫДАЧИ: '

 DO CASE 
  CASE kol_pv==0
   _SCREEN.Caption = _SCREEN.Caption + 'НЕ ВЫБРАН НИ ОДНИ ПУНКТ!'
  CASE kol_pv==1
...
*ppath = pbase+'\'+pvid(1,1)
 ...
 goPSD.Interval = 10*1000 && Интервал поиска файла ShutDown.txt
 goPSD.SystemShutDownFile = m.pBase+'\ShutDown.txt'
 goPSD.ShutDownWaitTime = 10*1000 && Время ожидания действия пользователя
