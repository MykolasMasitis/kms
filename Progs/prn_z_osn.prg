PROCEDURE prn_z_osn
 ...
 dotname = pTempl + "\z_osn.dot"
 docname = pLocal + "\z_osn"
 ...
 oDoc.Bookmarks('sn_card').Select  
 oWord.Selection.TypeText(m.enp)

 m.IsEnp = IIF(EMPTY(m.enp), 'V', '')
 oDoc.Bookmarks('IsEnp').Select  
 oWord.Selection.TypeText(m.IsEnp)
